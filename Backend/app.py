...
def get_expenses():
    ...
    conn.close()
    expenses = [{"id": r["id"], "amount": r["amount"], "category": r["category"], "note": r["note"], "date": r["date"]} for r in rows]
...
def add_expense():
    ...
    conn = get_db()
    conn.execute("INSERT INTO expenses (amount, category, note, date) VALUES (?, ?, ?, ?)", (amount, category, note, date))
    conn.commit()
    conn.close()
